...
/// @brief Used to descriptively refer to the value given to glove and dongle IDs when they are
/// uninitialized.
#define UNINITIALIZED_ID 0

/// @brief Used to descriptively refer to the maximum number of hosts that we support when finding
/// manus core.
...
#define MAX_NUMBER_OF_DONGLES \
  16  // we only have 35 radio channels available. if we got 16 dongles, expect wireless issues! and
      // no 16 usb ports is also not likely.
...
#define MAX_NUM_CHARS_IN_NODE_NAME \
  256  // this is for a UTF8 string , NOT an ASCII CHAR array (same base type though)
...
/// @brief Used to descriptively refer to the max number of fingers we support per hand in chains
/// (not the same as a glove, as we can retarget alien hands with more or less fingers). Used with
/// arrays and loops to make them more descriptive than simply using the number, and to make
/// changing the number easier and safer.
#define MAX_NUM_FINGER_IDS 10

/// @brief Used to descriptively refer to the max number of toes we support per foot in chains ( we
/// can retarget alien feet with more or less toes). Used with arrays and loops to make them more
/// descriptive than simply using the number, and to make changing the number easier and safer.
...
#define MAX_NUMBER_OF_SESSIONS \
  8  // this is not the real limit for Core but just for the SDKClient to handle
...
typedef enum SDKReturnCode {
  ...
  /// the stub has been reset but someone is tryign to use it anyway. usually after a shutdown of
  /// the SDK.
...
typedef enum MeasurementType {
  ...
  MeasurementType_MAX_SIZE  // Warning, this value is used to define the
                            // UserProfile.Measurement[SIZE]
} MeasurementType;
...
typedef enum TrackerOffsetType {
  ...
  TrackerOffsetType_MAX_SIZE  // Warning, this value is used to define the
                              // UserProfile.TrackerOffset[SIZE]
} TrackerOffsetType;
...
typedef enum ExtraTrackerOffsetType {
  ...
  ExtraTrackerOffsetType_MAX_SIZE  // Warning, this value is used to define the
                                   // UserProfile.TrackerOffset[SIZE]
...
/// @brief Describes the different types of body measurement categories used for the polygon
/// calibration.
...
/// @brief Describes the settings that can be applied to a node, it is defined as a flag so that
/// more than one setting can be set.
...
typedef enum Side { Side_Invalid, Side_Left, Side_Right, Side_Center } Side;
...
/// The transform is defined as a local or global transform depending on the coordinate system set
/// when initializing the SDK. See functions CoreSdk_InitializeCoordinateSystemWithVUH and
...
typedef struct ExtraTrackerOffset {
  ExtraTrackerOffsetType
      entryType;  // default = ExtraTrackerOffsetType::ExtraTrackerOffsetType_Unknown;
  float value;    // default = 0.0f;
} ExtraTrackerOffset;
...
// Skeleton
// ------------------------------------------------------------------------------------------------------------------------------
/// @brief Stores the inverse kinematics settings of a node, the ikAim is used to control the IK
/// solve, 1 is default, -1 is inversed.
typedef struct NodeSettingsIK {
  float ikAim;
} NodeSettingsIK;

/// @brief Stores the settings for a node of type foot, heightFromGround is used to set the height
/// of the 3d model ankle from ground.
...
/// @brief Stores the settings of a node of type leaf, the direction vector is defined with respect
/// to the previous node in the chain.
...
/// @brief Stores the node setup information. Each node represents a segment of the skeleton that
/// can be animated, nodes combine together to form chains. the parentID is used to identify the
/// node with respect to which the current one will move.
typedef struct NodeSetup {
  uint32_t id;                            // default = 0;
  char name[MAX_NUM_CHARS_IN_NODE_NAME];  // this is a UTF8 string , NOT an ASCII CHAR array (same
                                          // base type though)
...
typedef struct SkeletonSetupInfo {
  ...
  char name[MAX_NUM_CHARS_IN_SKELETON_NAME];  // this is a UTF8 string , NOT an ASCII CHAR array
                                              // (same base type though)
} SkeletonSetupInfo;
...
/// @brief Stores the information regarding the coordinate system used by the client, defined as VUH
/// (view, up, handedness).
...
/// @brief Stores the information regarding the coordinate system used by the client, defined by
/// each axis direction.
...
typedef void (*LoggingCallback_t)(LogSeverity p_Severity,
                                  const char* const p_Log,
                                  uint32_t p_Length);
