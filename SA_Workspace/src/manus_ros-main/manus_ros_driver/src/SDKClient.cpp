...
SDKClient::SDKClient() {
  ...
  glove_pub_left =
      node.advertise<std_msgs::Float32MultiArray>("manus_glove_data_left", 1);
  glove_pub_right =
      node.advertise<std_msgs::Float32MultiArray>("manus_glove_data_right", 1);
  TestTimestamp();
}
...
/// This function attempts to resize the console window and then proceeds to initialize the SDK's
/// interface.
ClientReturnCode SDKClient::Initialize() {
  ...
  if (t_IntializeResult != ClientReturnCode::ClientReturnCode_Success) {
    ClientLog::error(
        "Failed to initialize the SDK. Are you sure the correct ManusSDKLibary is used?");
...
ClientReturnCode SDKClient::Run() {
  ...
  while (!m_RequestedExit) {
    ...
    // in this example SDK Client we have several phases during our main loop to make sure the SDK
    // is in the right state to work.
...
/// after this is called it is expected to exit the client program. If not it needs to call
/// initialize again.
ClientReturnCode SDKClient::ShutDown() {
  ...
  if (t_Result != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to shut down the SDK wrapper. The value returned was {}.",
                     (int32_t)t_Result);
...
void SDKClient::OnConnectedCallback(const ManusHost* const p_Host) {
  ...
  const SDKReturnCode t_Result =
      CoreSdk_GetVersionsAndCheckCompatibility(&t_SdkVersion, &t_CoreVersion, &t_IsCompatible);

  if (t_Result == SDKReturnCode::SDKReturnCode_Success) {
    const std::string t_Versions = "Sdk version : " + std::string(t_SdkVersion.versionInfo) +
    ...
    ClientLog::error("Failed to get the versions from the SDK. The value returned was {}.",
                     (int32_t)t_Result);
  }
  ...
  if (t_SessionIdResult == SDKReturnCode::SDKReturnCode_Success && t_SessionId != 0) {
    ...
    ClientLog::print("Failed to get the Session ID from Core. The value returned was{}.",
                     (int32_t)t_SessionIdResult);
  }

  ManusHost t_Host(*p_Host);
  s_Instance->m_Host = std::make_unique<ManusHost>(t_Host);

  // Set the hand motion mode of the RawSkeletonStream. This is optional and can be set to any of
  // the HandMotion enum values. Default = None
  ...
  if (t_HandMotionResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::print("Failed to set the hand motion mode. The value returned was {}.",
...
/// @param p_SkeletonStreamInfo contains the meta data on how much data regarding the skeleton we
/// need to get from the SDK.
void SDKClient::OnSkeletonStreamCallback(const SkeletonStreamInfo* const p_SkeletonStreamInfo) {
  if (s_Instance) {
    ...
    for (uint32_t i = 0; i < p_SkeletonStreamInfo->skeletonsCount; i++) {
      ...
      t_NxtClientSkeleton->skeletons[i].nodes.resize(
      ...
      CoreSdk_GetSkeletonData(i, t_NxtClientSkeleton->skeletons[i].nodes.data(),
                              t_NxtClientSkeleton->skeletons[i].info.nodesCount);
    }
...
/// @brief This gets called when the client is connected to manus core. It sends the skeleton data
/// coming from the estimation system, before the retargeting to the client skeleton model.
/// @param p_RawSkeletonStreamInfo contains the meta data on how much data regarding the raw
/// skeleton we need to get from the SDK.
void SDKClient::OnRawSkeletonStreamCallback(
...
/// @param p_TrackerStreamInfo contains the meta data on how much data regarding the trackers we
/// need to get from the SDK.
  ...
  if (s_Instance) {
    for (uint32_t i = 0; i < p_GestureStream->gestureProbabilitiesCount; i++) {
      ...
      if (t_Probs.id != s_Instance->m_FirstLeftGloveID &&
          t_Probs.id != s_Instance->m_FirstRightGloveID)
        continue;
      ...
      for (uint32_t b = 0; b < t_BatchCount; b++) {
        ...
        CoreSdk_GetGestureStreamData(
            i, t_ProbabilityIdx,
            &t_Probs);  // this will get more data, if needed for the next iteration.
      }
...
void SDKClient::OnLandscapeCallback(const Landscape* const p_Landscape) {
  ...
  CoreSdk_GetGestureLandscapeData(s_Instance->m_NewGestureLandscapeData.data(),
                                  (uint32_t)s_Instance->m_NewGestureLandscapeData.size());
  ...
  if (s_Instance) {
    ...
    switch (p_SystemMessage->type) {
      case SystemMessageType::SystemMessageType_TemporarySkeletonModified:
        // if the message was triggered by a temporary skeleton being modified then save the
        // skeleton index, this information will be used to get and load the skeleton into core
...
float SDKClient::RoundFloatValue(float p_Value, int p_NumDecimalsToKeep) {
  ...
  float t_Power = static_cast<float>(std::pow(10.0, static_cast<double>(p_NumDecimalsToKeep)));
...
ClientReturnCode SDKClient::InitializeSDK() {
  ...
  ClientLog::print(
      "[1] Core Integrated - This will run standalone without the need for a MANUS Core "
      "connection");
  ClientLog::print(
  ...
  // however after initializing, the SDK is not yet connected to a host or doing anything network
  // related just yet.
  ...
  if (t_InitializeResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to initialize the Manus Core SDK. The value returned was {}.",
                     (int32_t)t_InitializeResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  // (each client can have their own settings. unreal and unity for instance use different
  // coordinate systems) if this is not set, the SDK will not connect to any Manus core host. The
  // coordinate system used for this example is z-up, x-positive, right-handed and in meter scale.
  ...
  // The above specified coordinate system is used to initialize and the coordinate space is
  // specified (world/local).
  const SDKReturnCode t_CoordinateResult = CoreSdk_InitializeCoordinateSystemWithVUH(t_VUH, true);

  /* this is an example if you want to use the other coordinate system instead of VUH (view, up,
  handedness) CoordinateSystemDirection t_Direction; t_Direction.x = AxisDirection::AD_Right;
  t_Direction.y = AxisDirection::AD_Up;
  t_Direction.z = AxisDirection::AD_Forward;
  const SDKReturnCode t_InitializeResult =
  CoreSdk_InitializeCoordinateSystemWithDirection(t_Direction, true);
  */

  if (t_CoordinateResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to initialize the Manus Core SDK coordinate system. The value returned was {}.",
        (int32_t)t_InitializeResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
...
ClientReturnCode SDKClient::RestartSDK() {
  ...
  if (t_ShutDownResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to shutdown the SDK. The value returned was {}.",
                     (int32_t)t_ShutDownResult);
    return ClientReturnCode::ClientReturnCode_FailedToShutDownSDK;
  }

  const ClientReturnCode t_IntializeResult = InitializeSDK();
  if (t_IntializeResult != ClientReturnCode::ClientReturnCode_Success) {
    ClientLog::error("Failed to initialize the SDK functionality. The value returned was {}.",
                     (int32_t)t_IntializeResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
...
/// Callbacks that are registered functions that get called when a certain 'event' happens, such as
/// data coming in from Manus Core. All of these are optional, but depending on what data you
/// require you may or may not need all of them.
ClientReturnCode SDKClient::RegisterAllCallbacks() {
  ...
  if (t_RegisterConnectCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for after connecting to Manus Core. The value "
        "returned was {}.",
        (int32_t)t_RegisterConnectCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  const SDKReturnCode t_RegisterDisconnectCallbackResult =
  ...
  if (t_RegisterDisconnectCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for after disconnecting from Manus Core. The value "
        "returned was {}.",
        (int32_t)t_RegisterDisconnectCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }

  // Register the callback for logging from the SDK
  // it is optional, but it might help display the logs at the right time/place
  const SDKReturnCode t_RegisterLogCallbackResult =
      CoreSdk_RegisterCallbackForOnLog(*OnLogCallback);
  if (t_RegisterLogCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for logging from the SDK. The value returned was {}.",
        (int32_t)t_RegisterLogCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  if (t_RegisterSkeletonCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for processing skeletal data from Manus Core. The "
        "value returned was {}.",
        (int32_t)t_RegisterSkeletonCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  const SDKReturnCode t_RegisterLandscapeCallbackResult =
  ...
  if (t_RegisterLandscapeCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback for landscape from Manus Core. The value returned was {}.",
        (int32_t)t_RegisterLandscapeCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }

  // Register the callback for when manus core is sending System messages
  // This is usually not used by client applications unless they want to show errors/events from
  // core. see OnSystemCallback for more details.
  const SDKReturnCode t_RegisterSystemCallbackResult =
  ...
  if (t_RegisterSystemCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for system feedback from Manus Core. The value "
        "returned was {}.",
        (int32_t)t_RegisterSystemCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  const SDKReturnCode t_RegisterErgonomicsCallbackResult =
  ...
  if (t_RegisterErgonomicsCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for ergonomics data from Manus Core. The value "
        "returned was {}.",
  ...
  if (t_RegisterRawSkeletonCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for processing raw skeletal data from Manus Core. "
  ...
  const SDKReturnCode t_RegisterTrackerCallbackResult =
  ...
  if (t_RegisterTrackerCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for processing tracker data from Manus Core. The "
        "value returned was {}.",
        (int32_t)t_RegisterTrackerCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
  ...
  const SDKReturnCode t_RegisterGestureCallbackResult =
      CoreSdk_RegisterCallbackForGestureStream(*OnGestureStreamCallback);
  if (t_RegisterGestureCallbackResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error(
        "Failed to register callback function for processing gesture data from Manus Core. The "
        "value returned was {}.",
        (int32_t)t_RegisterGestureCallbackResult);
    return ClientReturnCode::ClientReturnCode_FailedToInitialize;
  }
...
ClientReturnCode SDKClient::LookingForHosts() {
  ...
  const SDKReturnCode t_NumberResult =
      CoreSdk_GetNumberOfAvailableHostsFound(&m_NumberOfHostsFound);
  if (t_NumberResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get the number of available hosts. The error given was {}.",
                     (int32_t)t_NumberResult);
  ...
  const SDKReturnCode t_HostsResult =
      CoreSdk_GetAvailableHostsFound(m_AvailableHosts.get(), m_NumberOfHostsFound);
  if (t_HostsResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get the available hosts. The error given was {}.",
                     (int32_t)t_HostsResult);
...
ClientReturnCode SDKClient::PickingHost() {
  if (m_ConsoleClearTickCount == 0) {
    ...
    for (unsigned int t_HostNumber = 0; t_HostNumber < 10 && t_HostNumber < m_NumberOfHostsFound;
         t_HostNumber++) {
      ClientLog::print("[{}] hostname \"{}\", IP address \"{}\" Version {}.{}.{}", t_HostNumber,
...
/// @brief After a connection option was selected, the client will now try to connect to manus core
/// via the SDK.
ClientReturnCode SDKClient::ConnectingToCore() {
  ...
  if (t_ConnectResult == SDKReturnCode::SDKReturnCode_NotConnected) {
    ...
    return ClientReturnCode::ClientReturnCode_Success;  // Differentiating between error and no
                                                        // connect
  }
  if (t_ConnectResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to connect to Core. The error given was {}.",
                     (int32_t)t_ConnectResult);
...
ClientReturnCode SDKClient::DisplayingDataSkeleton() {
  ...
  ClientLog::print(
      "<Skeleton> [B] Load Left Hand Skeleton [N] Load Right Hand Skeleton [M] Unload Skeleton");
...
ClientReturnCode SDKClient::DisplayingDataTracker() {
  ...
  ClientLog::print(
      "[O] Toggle Test Tracker [G] Toggle per user tracker display [T] Set Tracker Offset to Left "
      "Wrist");
...
ClientReturnCode SDKClient::DisplayingDataTemporarySkeleton() {
  ...
  ClientLog::print(
      "<Skeleton>[B] Build Temporary Skeleton [C] Clear Temporary Skeleton [D] Clear All Temporary "
      "Skeletons For The Current Session");
  ClientLog::print(
...
ClientReturnCode SDKClient::DisplayingGloveCalibration() {
  ...
  ClientLog::print(
      "<Glove Calibration>[H] Toggle other Hand [P] Add 1 to the calibration step [M] Remove 1 "
      "from the calibration step");
  ClientLog::print(
      "<Glove Calibration>[S] Start glove calibration [C] Cancel glove calibration [F] Finish "
      "glove calibration");
...
ClientReturnCode SDKClient::DisconnectedFromCore() {
  ...
  auto t_Duration = std::chrono::duration_cast<std::chrono::seconds>(
                        std::chrono::high_resolution_clock::now() - m_TimeSinceLastDisconnect)
                        .count();
  ...
  if (t_ConnectLocally) {
    ...
    ClientLog::print("[R] Try to reconnect to the last host {} at {}.", m_Host->hostName,
                     m_Host->ipAddress);
    if (GetKeyDown('R')) {
      ClientLog::print("Reconnecting");

      ClientReturnCode t_ReconnectResult =
          ReconnectingToCore(m_SecondsToAttemptReconnecting, m_MaxReconnectionAttempts);
      if (t_ReconnectResult != ClientReturnCode::ClientReturnCode_FailedToConnect) {
        return t_ReconnectResult;
      }
    }
...
/// @brief It is called when the sdk is disconnected from Core and the user select one of the
/// options to reconnect.
ClientReturnCode SDKClient::ReconnectingToCore(int32_t p_ReconnectionTime,
  ...
  while ((p_ReconnectionAttempts > 0) && (p_ReconnectionTime > 0)) {
    ClientLog::print("Trying to reconnect to {} at {}. Attempt {}.", m_Host->hostName,
                     m_Host->ipAddress, t_Attempt);
    ClientLog::print("Attempts remaining: {}. Seconds before time out: {}.", p_ReconnectionAttempts,
                     p_ReconnectionTime);
...
void SDKClient::PrintHandErgoData(ErgonomicsData& p_ErgoData, bool p_Left) {
  const std::string t_FingerNames[NUM_FINGERS_ON_HAND] = {"[thumb] ", "[index] ", "[middle]",
                                                          "[ring]  ", "[pinky] "};
  ...
  std_msgs::Float32MultiArray glove_msg_left;
  std_msgs::Float32MultiArray glove_msg_right;
  ...
  if (p_Left) {
    for (int i = 0; i < 20; i++) {
      glove_msg_left.data.push_back(p_ErgoData.data[i]);
    }
    glove_pub_left.publish(glove_msg_left);
  } else {
    for (int i = 20; i < 40; i++) {
      glove_msg_right.data.push_back(p_ErgoData.data[i]);
    }
    glove_pub_right.publish(glove_msg_right);
  }
  ...
  for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++) {
    ClientLog::printWithPadding(
        "{} {} spread: {}, {} stretch: {}, {} stretch: {}, {} stretch: {} ", 6,
        t_FingerNames[t_FingerNumber],  // Name of the finger.
        t_JointNames[0], RoundFloatValue(p_ErgoData.data[t_DataOffset], 2), t_JointNames[0],
        RoundFloatValue(p_ErgoData.data[t_DataOffset + 1], 2), t_JointNames[1],
        RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2), t_JointNames[2],
...
void SDKClient::PrintErgonomicsData() {
  ...
  ClientLog::print(" -- Ergo Timestamp {}:{}:{}.{} ~ {}/{}/{}(D/M/Y)", m_ErgoTimestampInfo.hour,
                   m_ErgoTimestampInfo.minute, m_ErgoTimestampInfo.second,
                   m_ErgoTimestampInfo.fraction, m_ErgoTimestampInfo.day, m_ErgoTimestampInfo.month,
                   std::to_string(m_ErgoTimestampInfo.year));
...
void SDKClient::PrintDongleData() {
  ...
  for (uint32_t i = 0; i < t_DongleCount; i++) {
    ...
    ClientLog::print(" Type: {} - {}", ConvertDeviceClassTypeToString(t_DongleData.classType),
...
void SDKClient::PrintSystemMessage() {
  ...
  // ClientLog::print("Received System data:{} / code:{}", m_SystemMessage,
  //                  (int32_t)m_SystemMessageCode);
...
void SDKClient::PrintSkeletonData() {
  ...
  ClientLog::print("Received Skeleton data. skeletons:{} first skeleton id:{}",
                   m_Skeleton->skeletons.size(), (int32_t)m_Skeleton->skeletons[0].info.id);
...
void SDKClient::PrintRawSkeletonData() {
  ...
  ClientLog::print(
      "Received Skeleton glove data from Core. skeletons:{} first skeleton glove id:{}",
      m_RawSkeleton->skeletons.size(), m_RawSkeleton->skeletons[0].info.gloveId);
  ...
  // we need to know the number of nodes in the hierarchy, so we can initialize an array with the
  // correct size.
  SDKReturnCode t_Result = CoreSdk_GetRawSkeletonNodeCount(t_GloveId, t_NodeCount);
  if (t_Result != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get Raw Skeleton Node Count. The error given was {}.",
                     (int32_t)t_Result);
    return;
  }

  // now get the hierarchy data, this can be used to reconstruct the positions of each node in case
  // the user set up the system with a local coordinate system, and to know what each node is
  // exactly. having a node position defined as local means that this will be related to its parent
  ...
  if (t_Result != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get Raw Skeleton Hierarchy. The error given was {}.",
                     (int32_t)t_Result);
    return;
  }

  ClientLog::print("Fetched array with Raw Skeleton Hierarchy with {} nodes.",
                   (int32_t)t_NodeCount);
  AdvanceConsolePosition(2);
}
...
void SDKClient::PrintTrackerData() {
  ClientLog::print("Tracker test active: {}.",
                   m_TrackerTest);  // To show that test tracker is being sent to core
  ...
  ClientLog::print("Received Tracker data. number of received trackers:{} first tracker type:{}",
                   m_TrackerData->trackerData.size(),
                   (int32_t)m_TrackerData->trackerData[0].trackerType);
}
...
void SDKClient::PrintTrackerDataGlobal() {
  ...
  SDKReturnCode t_TrackerResult =
      CoreSdk_GetNumberOfAvailableTrackers(&t_NumberOfAvailabletrackers);
  if (t_TrackerResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get tracker data. The error given was {}.",
                     (int32_t)t_TrackerResult);
    return;
  }
...
void SDKClient::PrintTrackerDataPerUser() {
  ...
  for (uint32_t i = 0; i < t_NumberOfAvailableUsers; i++) {
    uint32_t t_NumberOfAvailabletrackers = 0;
    SDKReturnCode t_TrackerResult =
        CoreSdk_GetNumberOfAvailableTrackersForUserIndex(&t_NumberOfAvailabletrackers, i);
    if (t_TrackerResult != SDKReturnCode::SDKReturnCode_Success) {
      ClientLog::error("Failed to get tracker data. The error given was {}.",
                       (int32_t)t_TrackerResult);
      return;
    ...
    ClientLog::print("received available trackers for user index[{}] :{} ", i,
                     t_NumberOfAvailabletrackers);

    if (t_NumberOfAvailabletrackers == 0)
      return;  // nothing else to do.
    TrackerId* t_TrackerId = new TrackerId[t_NumberOfAvailabletrackers];
    t_TrackerResult =
        CoreSdk_GetIdsOfAvailableTrackersForUserIndex(t_TrackerId, i, t_NumberOfAvailabletrackers);
    if (t_TrackerResult != SDKReturnCode::SDKReturnCode_Success) {
      ClientLog::error("Failed to get tracker data. The error given was {}.",
                       (int32_t)t_TrackerResult);
      return;
...
void SDKClient::PrintLandscapeTimeData() {
  ...
  ClientLog::print("Current Interface: {} {} at index {}", m_Landscape->time.currentInterface.name,
  ...
  ClientLog::print("Fake signal: {} | Sync Pulse: {} | Sync Status: {}",
                   m_Landscape->time.fakeTimecode, m_Landscape->time.useSyncPulse,
                   m_Landscape->time.syncStatus);
  ClientLog::print("Device keep alive: {} | Timecode Status: {}", m_Landscape->time.deviceKeepAlive,
...
void SDKClient::PrintGestureData() {
  ...
  ClientLog::print("Total count of gestures for the {} glove: {}", t_Side,
                   t_Gest->info.totalGestureCount);
  ...
  for (uint32_t i = 0; i < t_Max; i++) {
    ...
    ClientLog::print("Gesture {} ({}) has a probability of {}%.", t_Name,
                     t_Gest->probabilities[i].id, t_Gest->probabilities[i].percent * 100.0f);
  }
...
/// @brief Prints the type of the first chain generated by the AllocateChain function, this is used
/// for testing.
...
/// @brief This support function checks if a temporary skeleton related to the current session has
/// been modified and gets it. Whenever the Dev Tools saves the changes to the skeleton, it sets the
/// boolean t_IsSkeletonModified to true for that skeleton. With callback OnSystemCallback this
/// application can be notified when one of its temporary skeletons has been modified, so it can get
/// it and, potentially, load it.
void SDKClient::GetTemporarySkeletonIfModified() {
  // if a temporary skeleton associated to the current session has been modified we can get it and,
  // potentially, load it
  if (m_ModifiedSkeletonIndex != UINT_MAX) {
    ...
    // At this point if we are satisfied with the modifications to the skeleton we can load it into
    // Core. Remember to always call function CoreSdk_ClearTemporarySkeleton after loading a
    // temporary skeleton, this will keep the temporary skeleton list in sync between Core and the
    // SDK.
...
/// @brief This support function gets the temporary skeletons for all sessions connected to Core and
/// it prints the total number of temporary skeletons associated to the current session. Temporary
/// skeletons are used for auto allocation and in the Dev Tools.
void SDKClient::PrintTemporarySkeletonInfo() {
  ...
  auto t_TimeSinceLastTemporarySkeletonUpdate =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::high_resolution_clock::now() - m_LastTemporarySkeletonUpdate)
          .count();
  if (t_TimeSinceLastTemporarySkeletonUpdate <
      static_cast<unsigned int>(MILLISECONDS_BETWEEN_TEMPORARY_SKELETONS_UPDATE)) {
    ClientLog::print("Total number of temporary skeletons in core: {} ",
                     t_TotalNumberOfTemporarySkeletonsInCore);
    return;
  }
  TemporarySkeletonCountForAllSessions t_TemporarySkeletonCountForAllSessions;
  SDKReturnCode t_Res =
      CoreSdk_GetTemporarySkeletonCountForAllSessions(&t_TemporarySkeletonCountForAllSessions);
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get all temporary skeletons. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }
  ...
  for (uint32_t i = 0; i < t_TemporarySkeletonCountForAllSessions.sessionsCount; i++) {
    t_TotalNumberOfTemporarySkeletonsInCore +=
        t_TemporarySkeletonCountForAllSessions.temporarySkeletonCountForSessions[i].skeletonCount;
  }
...
/// We have two ways of sending the vibration commands, based on the dongle id or based on the
/// skeleton id.
void SDKClient::HandleHapticCommands() {
  ...
  // The strange key number sequence here results from having gloves lie in front of you, and have
  // the keys and haptics in the same order.
  ...
  for (unsigned int t_HandNumber = 0; t_HandNumber < NUMBER_OF_HANDS_SUPPORTED; t_HandNumber++) {
    ...
    for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++) {
      t_HapticsPowers[t_FingerNumber] =
          t_HapticState[t_HandNumber].shouldHapticFinger[t_FingerNumber] ? s_FullPower : 0.0f;
    }
...
void SDKClient::HandleSkeletonHapticCommands() {
  ...
  const std::chrono::high_resolution_clock::time_point s_Now =
      std::chrono::high_resolution_clock::now();
  const long long s_MillisecondsSinceLastHapticCommand =
      std::chrono::duration_cast<std::chrono::milliseconds>(s_Now - s_TimeOfLastHapticsCommandSent)
          .count();

  if (s_MillisecondsSinceLastHapticCommand <
      static_cast<unsigned int>(MINIMUM_MILLISECONDS_BETWEEN_HAPTICS_COMMANDS)) {
    return;
  }
  ...
  for (unsigned int t_HandNumber = 0; t_HandNumber < NUMBER_OF_HANDS_SUPPORTED; t_HandNumber++) {
    ...
    if (CoreSdk_DoesSkeletonGloveSupportHaptics(m_Skeleton->skeletons[0].info.id,
                                                s_Hands[t_HandNumber], &t_IsHaptics) !=
    ...
    CoreSdk_VibrateFingersForSkeleton(m_Skeleton->skeletons[0].info.id, s_Hands[t_HandNumber],
                                      t_HapticsPowers);
  }
}
...
void SDKClient::HandleTrackerCommands() {
  ...
  if (m_TrackerTest) {
    ...
    if (t_TrackerSend != SDKReturnCode::SDKReturnCode_Success) {
      ClientLog::error("Failed to send tracker data. The error given was {}.",
                       (int32_t)t_TrackerSend);
      return;
    }
  }
}

/// @brief This support function is used to set the tracker offset. It sets the tracker offset for
/// the left hand tracker to the wrist.
void SDKClient::SetTrackerOffset() {
  TrackerOffset t_TrackerOffset;

  // Positions the tracker 3mm right, 58mm up and 43mm forward relative to the wrist. This value is
  // illustrative and should be adjusted to the actual tracker position.
  ...
  if (t_SetTrackerReturn != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to set tracker offset. The error given was {}.",
                     (int32_t)t_SetTrackerReturn);
    return;
  }
}
...
/// @param p_PosX X position of the node, this is defined with respect to the global coordinate
/// system or the local one depending on the parameter p_UseWorldCoordinates set when initializing
/// the sdk,
/// @param p_PosY Y position of the node this is defined with respect to the global coordinate
/// system or the local one depending on the parameter p_UseWorldCoordinates set when initializing
/// the sdk,
/// @param p_PosZ Z position of the node this is defined with respect to the global coordinate
/// system or the local one depending on the parameter p_UseWorldCoordinates set when initializing
/// the sdk,
/// @param p_Name the name of the node setup
/// @return the generated node setup
NodeSetup SDKClient::CreateNodeSetup(uint32_t p_Id,
                                     uint32_t p_ParentId,
                                     float p_PosX,
                                     float p_PosY,
                                     float p_PosZ,
...
/// In the case of this sample we create a Hand skeleton for which we want to get the calculated
/// result. The ID's for the nodes set here are the same IDs which are used in the
/// OnSkeletonStreamCallback, this allows us to create the link between Manus Core's data and the
/// data we enter here.
bool SDKClient::SetupHandNodes(uint32_t p_SklIndex, Side p_Side) {
  ...
  // Because the skeleton is build up in world space coordinates all joints have to be defined in
  // coordate space as well If it is required for the skeleton to work in local space, substract all
  // previous positions from the vectors
  static ManusVec3 s_LeftHandPositions[t_NumFingers * t_NumJoints]{
      ...
      CreateManusVec3(0.025320f + 0.032742f + 0.028739f + 0.028739f, -0.024950f,
                      0.000000f),  // Thumb Tip joint
      ...
      CreateManusVec3(0.052904f + 0.038257f + 0.020884f + 0.018759f, -0.011181f,
                      0.000000f),  // Index Tip joint
      ...
      CreateManusVec3(0.051287f + 0.041861f + 0.024766f + 0.019683f, 0.000000f,
                      0.000000f),  // Middle Tip joint
      ...
      CreateManusVec3(0.049802f + 0.039736f + 0.023564f + 0.019868f, 0.011274f,
                      0.000000f),  // Ring Tip joint
      ...
      CreateManusVec3(0.047309f + 0.033175f + 0.018020f + 0.019129f, 0.020145f,
                      0.000000f),  // Pinky Tip joint
  };
  ...
  // Note, these values are just an example of node positions and refer to the hand laying on a flat
  // surface.
  ManusVec3* t_Fingers;
  ...
  SDKReturnCode t_Res =
      CoreSdk_AddNodeToSkeletonSetup(p_SklIndex, CreateNodeSetup(0, 0, 0, 0, 0, "Hand"));
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to Add Node To Skeleton Setup. The error given was {}.",
                     (int32_t)t_Res);
    return false;
  }
  ...
  for (uint32_t i = 0; i < t_NumFingers; i++) {
    ...
    for (uint32_t j = 0; j < t_NumJoints; j++) {
      t_Res = CoreSdk_AddNodeToSkeletonSetup(
          p_SklIndex,
          CreateNodeSetup(1 + t_FingerId + j, t_ParentID, t_Fingers[i * 4 + j].x,
                          t_Fingers[i * 4 + j].y, t_Fingers[i * 4 + j].z, "fingerdigit"));
      if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
...
bool SDKClient::SetupHandChains(uint32_t p_SklIndex, Side p_Side) {
  // Add the Hand chain, this identifies the wrist of the hand
  {
    ...
    t_ChainSettings.hand.fingerChainIds[0] =
        1;  // links to the other chains we will define further down
    ...
    if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
      ClientLog::error("Failed to Add Chain To Skeleton Setup. The error given was {}.",
                       (int32_t)t_Res);
      return false;
    }
  }
  ...
  const ChainType t_FingerTypes[5] = {
      ChainType::ChainType_FingerThumb, ChainType::ChainType_FingerIndex,
  ...
  for (int i = 0; i < 5; i++) {
    ...
    // This identifies the metacarpal bone, if none exists, or the chain is a thumb it should be set
    // to -1. The metacarpal bone should not be part of the finger chain, unless you are defining a
    // thumb which does need it.
    t_ChainSettings.finger.metacarpalBoneId = -1;
    t_ChainSettings.finger.useLeafAtEnd =
        false;  // this is set to true if there is a leaf bone to the tip of the finger.
    ChainSetup t_Chain;
    ChainSetup_Init(&t_Chain);
    ...
    {
      t_Chain.nodeIdCount = 4;           // The amount of node id's used in the array
      t_Chain.nodeIds[0] = (i * 4) + 1;  // this links to the hand node created in the
                                         // SetupHandNodes
      t_Chain.nodeIds[1] = (i * 4) + 2;  // this links to the hand node created in the
                                         // SetupHandNodes
      t_Chain.nodeIds[2] = (i * 4) + 3;  // this links to the hand node created in the
                                         // SetupHandNodes
      t_Chain.nodeIds[3] = (i * 4) + 4;  // this links to the hand node created in the
                                         // SetupHandNodes
    }
    t_Chain.settings = t_ChainSettings;
...
void SDKClient::LoadTestSkeleton(Side p_Side) {
  ...
  CopyString(t_SKL.name, sizeof(t_SKL.name),
             p_Side == Side::Side_Left ? std::string("LeftHand") : std::string("RightHand"));
...
/// This function saves the temporary skeleton with the given session ID and returns an error if the
/// operation fails.
...
/// @brief This support function sets up an incomplete hand skeleton and then uses manus core to
/// allocate chains for it.
void SDKClient::AllocateChains() {
  ...
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to allocate chains for skeleton. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }
  ...
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get skeleton setup chains. The error given was {}.",
                     (int32_t)t_Res);
    delete[] t_Chains;
    return;
  }
  ...
  for (size_t i = 0; i < t_SkeletonInfo.chainsCount; i++) {
    if (t_Chains[i].dataType == ChainType::ChainType_Hand) {
      ...
      if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
        ClientLog::error("Failed to overwrite Chain To Skeleton Setup. The error given was {}.",
                         (int32_t)t_Res);
        delete[] t_Chains;
        return;
      }
...
/// @brief This support function is used for the manual allocation of the skeleton chains with means
/// of a temporary skeleton. Temporary Skeletons can be helpful when the user wants to modify the
/// skeletons chains or nodes more than once before retargeting, as they can be saved in core and
/// retrieved later by the user to apply further modifications. This can be done easily with means
/// of the Dev Tools. The current function contains an example that shows how to create a temporary
/// skeleton, modify its chains, nodes and skeleton info, and save it into Core. This process should
/// be used during development. After that, the final skeleton should be loaded into core for
/// retargeting and for getting the actual data,as displayed in the LoadTestSkeleton function.
void SDKClient::BuildTemporarySkeleton() {
  ...
  bool t_IsSkeletonModified = false;  // this bool is set to true by the Dev Tools after saving any
                                      // modification to the skeleton,
  // this triggers the OnSyStemCallback which is used in the SDK to be notified about a change to
  // its temporary skeletons. for the purpose of this example setting this bool to true is not
  // really necessary.
  ...
  t_Settings.scaleToTarget = true;
  t_Settings.targetType = SkeletonTargetType::SkeletonTargetType_UserData;
  t_Settings.skeletonTargetUserData.userID =
      0;  // this needs to be a real user Id when retargeting, when editing the temporary skeleton
          // this may (hopefully) not cause issues
  ...
  // if we want to modify the skeleton setup or if we want to apply some changes to the chains or
  // nodes: first overwrite the existing skeleton setup and then re-add all the chains and nodes to
  // it
  SkeletonSettings_Init(&t_Settings);
  ...
  // this way we overwrite the temporary skeleton with index t_SklIndex with the modified skeleton
  // setup
  ...
  for (size_t i = 0; i < t_SkeletonInfo.chainsCount; i++) {
    ...
    if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
      ClientLog::error("Failed to Add Chains To Skeleton Setup. The error given was {}.",
                       (int32_t)t_Res);
      return;
    }
  }
  ...
  // in the Dev Tools this bool is set to true when saving the temporary skeleton, this triggers
  // OnSystemCallback which notifies the SDK sessions about a modifications to one of their
  // temporary skeletons. setting the bool to true in this example is not really necessary, it's
...
/// @brief This support function is used to clear a temporary skeleton from the temporary skeleton
/// list, for example it can be used when the max number of temporary skeletons has been reached for
/// a specific session.
...
/// @brief This support function is used to clear all temporary skeletons associated to the current
/// SDK session, it can be used when the max number of temporary skeletons has been reached for the
/// current session and we want to make room for more.
void SDKClient::ClearAllTemporarySkeletons() {
  ...
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to Clear All Temporary Skeletons. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }
...
void SDKClient::SaveTemporarySkeletonToFile() {
  ...
  bool t_IsSkeletonModified =
      false;  // setting this bool to true is not necessary here, it is mostly used by the Dev Tools
  ...
  t_Res = CoreSdk_CompressTemporarySkeletonAndGetSize(t_SklIndex, t_SessionId,
                                                      &t_TemporarySkeletonLengthInBytes);
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to compress temporary skeleton and get size. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }
  unsigned char* t_TemporarySkeletonData = new unsigned char[t_TemporarySkeletonLengthInBytes];

  // get the array of bytes with the compressed temporary skeleton data, remember to always call
  // function CoreSdk_CompressTemporarySkeletonAndGetSize before trying to get the compressed
  // temporary skeleton data
  t_Res = CoreSdk_GetCompressedTemporarySkeletonData(t_TemporarySkeletonData,
                                                     t_TemporarySkeletonLengthInBytes);
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to get compressed temporary skeleton data. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }

  // now save the data into a .mskl file
  // as an example we save the temporary skeleton in a folder called ManusTemporarySkeleton inside
  // the documents directory get the path for the documents directory
  ...
  if (t_Res != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to Clear Temporary Skeleton after saving. The error given was {}.",
                     (int32_t)t_Res);
    return;
  }
  RemoveIndexFromTemporarySkeletonList(t_SklIndex);
...
void SDKClient::GetTemporarySkeletonFromFile() {
  ...
  // as an example we try to get the temporary skeleton data previously saved as .mskl file in
  // directory Documents/ManusTemporarySkeleton get the path for the documents directory
  std::string t_DirectoryPathString = GetDocumentsDirectoryPath_UTF8();
  ...
  // save the zipped temporary skeleton information, they will be used internally for sending the
  // data to Core
  ...
  // load the temporary skeleton data retrieved from the zipped file and save it with index
  // t_SklIndex and session id of the current session
  SDKReturnCode t_Result = CoreSdk_GetTemporarySkeletonFromCompressedData(
      t_SklIndex, t_SessionId, t_TemporarySkeletonData, t_TemporarySkeletonLengthInBytes);
  if (t_Result != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::warn(
        "Failed to load temporary skeleton data from client file in Core, the error code was: {}.",
        (int32_t)t_Result);
    return;
  }
...
void SDKClient::PairGlove() {
  ...
  if (t_PairGloveResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to pair glove with ID: {}. The error given was {}", t_GloveToPairId,
                     (int32_t)t_PairGloveResult);
    return;
  }
...
void SDKClient::UnpairGlove() {
  ...
  if (t_UnpairGloveResult != SDKReturnCode::SDKReturnCode_Success) {
    ClientLog::error("Failed to unpair glove with ID: {}. The error given was ", t_GloveToUnPairId,
                     (int32_t)t_UnpairGloveResult);
    return;
  }
