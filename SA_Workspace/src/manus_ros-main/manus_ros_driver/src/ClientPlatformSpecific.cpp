...
static void HandleTerminationSignal(int p_Parameter) {
  ClientLog::error("Termination signal sent with parameter {}.", p_Parameter);
...
static void HandleInterruptSignal(int p_Parameter) {
  ClientLog::error("Interrupt signal sent with parameter {}.", p_Parameter);
...
static void HandleQuitSignal(int p_Parameter) {
  ClientLog::error("Quit signal sent with parameter {}.", p_Parameter);
...
static void HandleHangupSignal(int p_Parameter) {
  ClientLog::error("Hang-up signal sent with parameter {}.", p_Parameter);
...
class ClientInput {
 public:
  ...
  void Update(void) {
    ...
    for (InputMap_t::iterator t_Key = m_PressedLastUpdate.begin();
         t_Key != m_PressedLastUpdate.end(); t_Key++) {
      t_Key->second = false;
    }

    // Copy the current state to the last update's state, and clear the
    // current state.
    for (InputMap_t::iterator t_Key = m_CurrentlyPressed.begin(); t_Key != m_CurrentlyPressed.end();
         t_Key++) {
      m_PressedLastUpdate[t_Key->first] = t_Key->second;
      t_Key->second = false;
    }
  ...
  bool GetKeyDown(const int p_Key) {
    ...
    const bool t_PreviousValue =
        t_PreviousState == m_PreviousKeyState.end() ? false : t_PreviousState->second;
...
/*static*/ bool SDKClientPlatformSpecific::CopyString(char* const p_Target,
                                                      const size_t p_MaxLengthThatWillFitInTarget,
                                                      const std::string& p_Source) {
  if (!p_Target) {
    ClientLog::error("Tried to copy a string, but the target was null. The string was \"{}\".",
                     p_Source.c_str());
  ...
  if (p_MaxLengthThatWillFitInTarget <= p_Source.length()) {
    ClientLog::error(
        "Tried to copy a string that was longer than {} characters, which makes it too big for its "
        "target buffer. The string was \"{}\".",
        p_MaxLengthThatWillFitInTarget, p_Source.c_str());
...
bool SDKClientPlatformSpecific::ResizeWindow(const short int p_ConsoleWidth,
...
void SDKClientPlatformSpecific::ApplyConsolePosition(const int p_ConsoleCurrentOffset) {
...
std::string SDKClientPlatformSpecific::GetDocumentsDirectoryPath_UTF8(void) {
  ...
  const std::string t_DocumentsDir = (!t_Xdg || strlen(t_Xdg) == 0)
