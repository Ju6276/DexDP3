...
/// @brief  Constant expression used to define the time between two updates of the temporary
/// skeleton count printing
...
/// @brief Used to store the information about the skeleton data coming from the estimation system
/// in Core.
...
class SDKClient : public SDKClientPlatformSpecific {
 public:
  ...
  ros::Publisher glove_pub_left;
  ros::Publisher glove_pub_right;
 ...
 protected:
  ...
  virtual ClientReturnCode ReconnectingToCore(int32_t p_ReconnectionTime = 0,
  ...
  static NodeSetup CreateNodeSetup(uint32_t p_Id,
                                   uint32_t p_ParentId,
                                   float p_PosX,
                                   float p_PosY,
                                   float p_PosZ,
